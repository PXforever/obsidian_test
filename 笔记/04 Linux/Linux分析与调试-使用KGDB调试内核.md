...
```
![[Linux分析与调试-使用KGDB调试内核/image-20240720180341422.png|Linux分析与调试-使用KGDB调试内核/image-20240720180341422.png]]
## 主机设备配置
...
## 本地串口调试
进入调试后状态如下：
![[Linux分析与调试-使用KGDB调试内核/image-20240725222910395.png|Linux分析与调试-使用KGDB调试内核/image-20240725222910395.png]]
我们使用`help`就可以看到命令列表：
```shell
...
```
只找到一个版本：
![[Linux分析与调试-使用KGDB调试内核/image-20240721113443980.png|Linux分析与调试-使用KGDB调试内核/image-20240721113443980.png]]
...
ls
```
![[Linux分析与调试-使用KGDB调试内核/image-20240721113812002.png|Linux分析与调试-使用KGDB调试内核/image-20240721113812002.png]]
...
## 调试
### 复制虚拟机
![[Linux分析与调试-使用KGDB调试内核/image-20240721114613223.png|Linux分析与调试-使用KGDB调试内核/image-20240721114613223.png]]
### 配置串口
`target`设置如下：
![[Linux分析与调试-使用KGDB调试内核/image-20240721114702362.png|Linux分析与调试-使用KGDB调试内核/image-20240721114702362.png]]
`host`设置如下：
![[Linux分析与调试-使用KGDB调试内核/image-20240721114735224.png|Linux分析与调试-使用KGDB调试内核/image-20240721114735224.png]]
...
`target`设置如下：
![[Linux分析与调试-使用KGDB调试内核/image-20240721120024390.png|Linux分析与调试-使用KGDB调试内核/image-20240721120024390.png]]
在这里设备就会卡住，我们需要操作`host`端。
`host`端设置如下：
![[Linux分析与调试-使用KGDB调试内核/image-20240721120246202.png|Linux分析与调试-使用KGDB调试内核/image-20240721120246202.png]]
这样便进入了`KGDB`调试。
![[Linux分析与调试-使用KGDB调试内核/image-20240721120420480.png|Linux分析与调试-使用KGDB调试内核/image-20240721120420480.png]]
...
```
接着会进入：
![[Linux分析与调试-使用KGDB调试内核/image-20240725205515146.png|Linux分析与调试-使用KGDB调试内核/image-20240725205515146.png]]
我们输入help：
![[Linux分析与调试-使用KGDB调试内核/image-20240725205541869.png|Linux分析与调试-使用KGDB调试内核/image-20240725205541869.png]]
这表示可以开始调试了。
## Host接入调试
我们打开`ubuntu`虚拟机，开启后，将串口接入到虚拟机：
![[Linux分析与调试-使用KGDB调试内核/image-20240725205958383.png|Linux分析与调试-使用KGDB调试内核/image-20240725205958383.png]]
通过：
```shell
dmesg | grep tty
```
![[Linux分析与调试-使用KGDB调试内核/image-20240725210055929.png|Linux分析与调试-使用KGDB调试内核/image-20240725210055929.png]]
可以看到`ttyUSB0`是该调试串口
我们进入到源码目录：
```shell
cd SDK_SOURCE
...
```
![[Linux分析与调试-使用KGDB调试内核/image-20240725210347713.png|Linux分析与调试-使用KGDB调试内核/image-20240725210347713.png]]
我们设置波特率和串口：
```shell
set serial baud 115200
target remote /dev/ttyUSB0
```
![[Linux分析与调试-使用KGDB调试内核/image-20240725210553523.png|Linux分析与调试-使用KGDB调试内核/image-20240725210553523.png]]
...
执行上面命令后`target`会卡住。
我们看看`host`端，发现触发中断了：
![[Linux分析与调试-使用KGDB调试内核/image-20240725220151807.png|Linux分析与调试-使用KGDB调试内核/image-20240725220151807.png]]
`gdb`中输入命令`list`可以查看一下源码：
![[Linux分析与调试-使用KGDB调试内核/image-20240725220229168.png|Linux分析与调试-使用KGDB调试内核/image-20240725220229168.png]]
对比实际的源码：
![[Linux分析与调试-使用KGDB调试内核/image-20240725220304358.png|Linux分析与调试-使用KGDB调试内核/image-20240725220304358.png]]
一模一样。
好了，我们开始单步调试：
```shell
(gdb) n
...
(gdb) list
```
![[Linux分析与调试-使用KGDB调试内核/image-20240725221220630.png|Linux分析与调试-使用KGDB调试内核/image-20240725221220630.png]]
我们查看一下指针`i2c`的地址，但是：
![[Linux分析与调试-使用KGDB调试内核/image-20240725221300762.png|Linux分析与调试-使用KGDB调试内核/image-20240725221300762.png]]
这里是被优化了，我们来看看`i2c_status`：
![[Linux分析与调试-使用KGDB调试内核/image-20240725221332550.png|Linux分析与调试-使用KGDB调试内核/image-20240725221332550.png]]
我们直接使用：
```shell
(gdb) r
```
`target`显示：
![[Linux分析与调试-使用KGDB调试内核/image-20240725221533662.png|Linux分析与调试-使用KGDB调试内核/image-20240725221533662.png]]
我们可以`print *attr`:
![[Linux分析与调试-使用KGDB调试内核/image-20240725221742862.png|Linux分析与调试-使用KGDB调试内核/image-20240725221742862.png]]
或打印`print *dev`：
![[Linux分析与调试-使用KGDB调试内核/image-20240725221825364.png|Linux分析与调试-使用KGDB调试内核/image-20240725221825364.png]]
# 示例4(调试用户定义模块)
...
# 地址最小的那个就是
```
![[Linux分析与调试-使用KGDB调试内核/image-20240727210810464.png|Linux分析与调试-使用KGDB调试内核/image-20240727210810464.png]]
#### 方式3
```shell
...
cat .text
```
![[Linux分析与调试-使用KGDB调试内核/image-20240727211524703.png|Linux分析与调试-使用KGDB调试内核/image-20240727211524703.png]]
![[Linux分析与调试-使用KGDB调试内核/image-20240727211638563.png|Linux分析与调试-使用KGDB调试内核/image-20240727211638563.png]]
### 进入调试
#### target进入
...
```
![[Linux分析与调试-使用KGDB调试内核/image-20240727214332000.png|Linux分析与调试-使用KGDB调试内核/image-20240727214332000.png]]
然后设置波特率参数，接着载入当前驱动的模块文件：
```shell
#地址是上面获取的
add-symbol-file lesson_4.ko 0xbf024000
```
![[Linux分析与调试-使用KGDB调试内核/image-20240727214655457.png|Linux分析与调试-使用KGDB调试内核/image-20240727214655457.png]]
我们设置一个断点：
```shell
(gdb) b scull_write 
```
然后使用`tty`连接调试设备，并继续运行设备：`c`。
![[Linux分析与调试-使用KGDB调试内核/image-20240727215221231.png|Linux分析与调试-使用KGDB调试内核/image-20240727215221231.png]]
接着在`target`上操作：
```shell
# 驱动这里并没有显示的创建文件节点，所以我们先看看scull设备的主设备号
dmesg
```
![[Linux分析与调试-使用KGDB调试内核/image-20240727220250030.png|Linux分析与调试-使用KGDB调试内核/image-20240727220250030.png]]
然后再创建一个文件节点：
```shell
mknode /dev/scull0 c 241 0
```
![[Linux分析与调试-使用KGDB调试内核/image-20240727220339330.png|Linux分析与调试-使用KGDB调试内核/image-20240727220339330.png]]
接着：
```shell
echo 9 > /dev/scull0
```
